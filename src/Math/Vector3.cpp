...
Vector3 Vector3::cross_product(const Vector3 arg) const {
  return Vector3(y * arg.z - z * arg.y, z * arg.x - x * arg.z,
                 x * arg.y - y * arg.x);
}

double Vector3::length() const { return sqrt(x * x + y * y + z * z); }

Vector3 Vector3::normalized() const { return *this / this->length(); }

Vector3 operator*(double arg1, Vector3 arg2) { return arg2 * arg1; }
