...
Ray Render::ray_from_screen_cords(Scene *scene, const Vector2 cords) {
  Vector3 screen_position = scene->get_camera()->screen_position;
  Vector2 screen_size = scene->get_camera()->screen_size;
  double x_offset = cords.x / window.size.x;
  double y_offset = -cords.y / window.size.y;
  Vector3 pos =
      Vector3(screen_position.x, screen_position.y + x_offset * screen_size.x,
              screen_position.z + y_offset * screen_size.y);
